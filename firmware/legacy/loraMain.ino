...
void setup() {
  ...
  SerialUSB.println("Serial Port Open - LORAWAN MINTS");
  ...
  SerialUSB.println("Initializing INA Battery");
  inaBat.begin();

  SerialUSB.println("Initializing INA Solar");
  inaSol.begin();
  float batVoltage = inaBat.getBusVoltage_V();
  float solVoltage = inaSol.getBusVoltage_V();
  SerialUSB.println("INAs Initialized");
  
  SerialUSB.println("Checking Battery Status");
  checkBatteryStatus(batVoltage, solVoltage);
 
  SerialUSB.println("Initializing GPS");
  gpsInit();
  
  SerialUSB.println("Initializing Dust Sensor");
  dustInit();
  
  SerialUSB.println("Initializing Air Sensor");
  airSensor.begin(); 
  airSensor.setMeasurementInterval(30);
  
  SerialUSB.println("Initializing BME280");
  baro.init();

  SerialUSB.println("Initializing MGS Sensor");
  gasInit();
